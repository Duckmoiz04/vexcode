...
export interface HeaderProps {
  ...
  theme: 'dark' | 'light';
  onToggleTheme: () => void;
  activeTab?: 'overview' | 'issues' | 'graph' | 'activity';
  onTabChange?: (tab: 'overview' | 'issues' | 'graph' | 'activity') => void;
...
export const Header: React.FC<HeaderProps> = ({
  ...
  if (!projectName) {
    return (
      <header className="w-full pt-10 pb-5 bg-bg-secondary flex items-center justify-center select-none font-display">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xl font-bold tracking-widest uppercase text-text-primary">Vexcode</span>
        </div>
      </header>
    );
  }
  ...
  return (
    <header className="flex items-center justify-between h-14 px-[18px] bg-bg-secondary border-b border-card-border z-40 font-display">
      <HeaderNav
